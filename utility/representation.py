...
from sprites_datagen.rewards import ActualReward
import numpy as np
...
class MainModule():
    ...
    def do(self, recon_epoch=10):
        ...
        for epoch in range(self.num_epoch):
            ...
            rewards = traj.rewards["tru_reward"]
            acc_reward_loss = 0
            acc_recon_loss = 0
            ...
            for t in range(0, len(torch_images)-self.num_enc-self.prediction_horizon):
                ...
                #observation = torch.stack(torch_images[t:t+self.num_enc].squeeze(0))
                observation = torch_images[t:t+self.num_enc].squeeze(1)
        ...
        save_path = save_dir + f'/encoder_weights_{current_time}.pth'
        torch.save(m.model.encoder.state_dict(), save_path)
...
if __name__ == "__main__":
    m=MainModule(num_epoch=50, ww=15, neck = 64)
    m.do(recon_epoch=40)
